...
export function calculateWalletMetrics(trades, priceMap = {}) {
    ...
    Object.values(tokenStats).forEach(stats => {
        ...
        // Handle Price Map (now supports Object { price, pairCreatedAt } or Number)
        let tokenPrice = 0;
        let pairCreated = null;

        if (priceMap[stats.mint]) {
            if (typeof priceMap[stats.mint] === 'object') {
                tokenPrice = priceMap[stats.mint].price;
                pairCreated = priceMap[stats.mint].pairCreatedAt;
            } else {
                tokenPrice = priceMap[stats.mint];
            }
        }
