...
export function calculateWalletMetrics(trades, priceMap = {}, solPrice = 150) {
    ...
    const IGNORED_MINTS = [
        ...
        // Add other noise tokens here
    ];

    for (const trade of trades) {
        if (IGNORED_MINTS.includes(trade.mint)) continue;

        // Also skip generic "USD" if symbol checks were possible here, but we only have mint.
        // We will filter purely by Mint for now.
    ...
    Object.values(tokenStats).forEach(stats => {
        ...
        // 1. Calculate Realized PnL on SOLD portion (Partial Wins/Losses)
        // Previous logic only counted Realized PnL if status === 'CLOSED'.
        // New logic: Any sold token contributes to Realized PnL.
        let realizedPnL = 0;
        let realizedPnLUSD = 0;

        if (sold > 0) {
            const avgBuyPrice = bought > 0 ? stats.totalBuySol / bought : 0;
            const avgSellPrice = stats.totalSellSol / sold;

            // Realized PnL = (AvgSell - AvgBuy) * SoldTokens
            // Simplified: TotalSellSol - (AvgBuy * SoldTokens)
        ...
        // 2. Unrealized PnL on REMAINING portion
        let unrealizedPnL = 0;
        ...
        if (remaining > 0) {
            // Even if tokenPrice is 0 (Rugged/No Data), calculate Value as 0 to show the Loss
            currentValue = remaining * tokenPrice;
            currentValueUSD = currentValue * solPrice;
        ...
        if (!isOrphan) {
            ...
            if (realizedPnL > 0) {
                ...
                grossProfitUSD += realizedPnLUSD;
            } else if (realizedPnL < 0) { // Only count as loss if actual realized loss
                lossCount++;
                grossLoss += Math.abs(realizedPnL);
                grossLossUSD += Math.abs(realizedPnLUSD);
            }

            // Duration & Valid Closed Count
            // We count 'validClosed' if mostly closed or significant duration? 
            // Stick to existing logic: only count duration stats for CLOSED positions.
