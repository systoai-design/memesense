 ...
 */
export function calculateWalletMetrics(trades, priceMap = {}) {
    ...
    Object.values(tokenStats).forEach(stats => {
        ...
        if (bought > 0 && (remaining / bought) < 0.02) {
            ...
            status = 'PARTIAL_HISTORY'; // "Grandma trade" - sold something we didn't see buy for
            // For PnL purposes, we might treat cost basis as 0 (pure profit) or ignore?
            // Let's count it as profit (Realized) but mark it.
            status = 'CLOSED'; // Treat as closed for winrate?
            isClosed = true;
        }
        ...
        let unrealizedPnL = 0;
        let currentValue = 0;
        if (priceMap[stats.mint] && remaining > 0) {
            currentValue = remaining * priceMap[stats.mint];
        ...
        allPositions.push({
            mint: stats.mint,
            status,
            pnl: isClosed ? cashflowPnL : (unrealizedPnL !== 0 ? unrealizedPnL : cashflowPnL),
            realizedPnL: isClosed ? cashflowPnL : 0,
            unrealizedPnL: isClosed ? 0 : unrealizedPnL,
            roi: stats.totalBuySol > 0 ? (cashflowPnL / stats.totalBuySol) * 100 : 0,
            duration,
 ...
 * @param {Array} trades 
 */
export function analyzeTimeWindows(trades, priceMap = {}) {
    const now = Date.now();
    ...
    return {
        '1d': calculateWalletMetrics(trades1d, priceMap),
        '7d': calculateWalletMetrics(trades7d, priceMap),
        '14d': calculateWalletMetrics(trades14d, priceMap),
        '30d': calculateWalletMetrics(trades30d, priceMap),
        'all': calculateWalletMetrics(trades, priceMap)
    };
}
