...
export async function getWalletSwaps(walletAddress, options = {}) {
    ...
    // Convert Solscan format to our standard trade format (async for metadata)
    return await parseSwapsToTrades(allActivities, walletAddress);
}

// Simple in-memory cache for token decimals (session-based)
...
/**
 * Batch fetch metadata for tokens
 */
async function fetchMissingDecimals(mints) {
    const missing = mints.filter(m => !DECIMAL_CACHE.has(m));
    if (missing.length === 0) return;

    console.log(`[Solscan] Fetching decimals for ${missing.length} tokens...`);
    ...
    for (let i = 0; i < missing.length; i += CHUNK_SIZE) {
        ...
        await Promise.all(chunk.map(async (mint) => {
            try {
                // Try cache first (redundant check but safe)
                if (DECIMAL_CACHE.has(mint)) return;

                const meta = await getTokenMeta(mint);
                if (meta && meta.decimals !== undefined) {
                    DECIMAL_CACHE.set(mint, meta.decimals);
                } else {
                    DECIMAL_CACHE.set(mint, null); // Mark as failed/unknown (will fallback to 9)
                }
            } catch (e) {
                console.warn(`[Solscan] Failed to fetch meta for ${mint}:`, e.message);
                DECIMAL_CACHE.set(mint, null);
            }
        }));
        // Small delay between chunks
        if (i + CHUNK_SIZE < missing.length) await new Promise(r => setTimeout(r, 200));
    }
}
 ...
 */
async function parseSwapsToTrades(activities, walletAddress) {
    const trades = [];
    ...
    // PRE-PASS: Collect all unique mints from relevant routers
    const mintsToFetch = new Set();

    // Helper to safely add mint
    const addSafe = (m) => {
        if (m && !DECIMAL_CACHE.has(m) && !STABLECOINS.has(m) && !ALTCOIN_BLOCKLIST.has(m)) {
            mintsToFetch.add(m);
        }
    };

    for (const activity of activities) {
        if (!activity.routers || activity.routers.length === 0) continue;
        const first = activity.routers[0];
        const last = activity.routers[activity.routers.length - 1];
        addSafe(first.token1);
        addSafe(last.token2);
    }

    // Fetch decimals for unknown tokens
    if (mintsToFetch.size > 0) {
        await fetchMissingDecimals(Array.from(mintsToFetch));
    }
    ...
    for (const activity of activities) {
        try {
            ...
            // RESOLVE DECIMALS: Use Cache first, then API response (if provided), default to 9
            const decimalsIn = DECIMAL_CACHE.get(tokenIn) ?? firstRouter.token1_decimals ?? 9;
            const decimalsOut = DECIMAL_CACHE.get(tokenOut) ?? lastRouter.token2_decimals ?? 9;

            const amountIn = parseFloat(firstRouter.amount1) / Math.pow(10, decimalsIn);
            const amountOut = parseFloat(lastRouter.amount2) / Math.pow(10, decimalsOut);
