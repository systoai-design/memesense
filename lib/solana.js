...
const maskedRpc = RPC_ENDPOINT.replace(/api-key=[a-zA-Z0-9-]+/, 'api-key=***');
console.log('Solana Lib Initialized with RPC:', maskedRpc);

// SPL Token Imports (Optimized)
import { getAssociatedTokenAddressSync, AccountLayout, TOKEN_PROGRAM_ID } from '@solana/spl-token';
...
async function checkHoldingStatus(walletList, tokenAddress) {
    ...
    // 1. Derive ATAs locally (synchronous, instant)
    ...
    for (const wallet of walletList) {
        try {
            ...
            const ata = getAssociatedTokenAddressSync(pubKey, owner);
            atas.push(ata);
            ataMap.set(ata.toBase58(), wallet);
        } catch (e) {
    ...
    // 2. Batch Fetch Accounts (Up to 100 per call)
    // We break 'atas' into chunks of 100
    const chunkSize = 100;
            ...
            for (const info of accountInfos) {
                if (info) {
                    ...
                    try {
                        ...
                        // Amount is a BigInt (u64)
                        // AccountLayout: mint(32), owner(32), amount(8), ...
                        const amount = Number(data.amount); // Safe for meme tokens usually, but strictly should be BigInt

                        // We assume anything > 10 "units" (raw) is holding.
                        // Usually decimals are 6 or 9. 1 token = 1e6 or 1e9. 
                        // So checking > 10 raw units is effectively > 0.000001 token.
                        if (amount > 1000) {
                            stillHolding++;
                        } else {
                            soldAll++;
                        ...
                        // Fallback if layout fails (shouldn't happen for standard SPL)
                        stillHolding++;
                    }
                } else {
                    // Account does not exist == Sold/Closed
                    soldAll++;
                }
            }
        } catch (e) {
            console.error('Error fetching batch ATAs:', e);
            // Assume 50/50 split on error? Or all sold?
            // Safer to assume soldAll to show higher risk?
            soldAll += chunk.length;
        }
    }
...
async function getSnipersFromRPC(tokenAddress) {
    try {
        ...
        const walletList = sniperWallets.slice(0, 10);
        return await checkHoldingStatus(walletList, tokenAddress);
    } catch (error) {
