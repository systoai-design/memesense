...
export async function getSniperData(tokenAddress) {
    // Method 0: Sniper Detector Engine (DISABLED - Causes hangs on high volume tokens due to recent block scanning)
    /*
    try {
...
async function getSnipersFromHelius(tokenAddress, apiKey) {
    try {
        const fetchWithFallback = async (key) => {
            const url = `https://api.helius.xyz/v0/addresses/${tokenAddress}/transactions?api-key=${key}&limit=100`;
        ...
        let response;
        try {
            response = await fetchWithFallback(apiKey);
        } catch (e) {
            const backupKey = process.env.HELIUS_BACKUP_API_KEY;
            if (backupKey && backupKey !== apiKey) {
                console.log('Primary Helius key failed, using backup...');
                response = await fetchWithFallback(backupKey);
            } else {
                throw e;
            }
        }
...
export async function getUniqueBuyers(tokenAddress) {
    try {
        ...
        // OPTIMIZED: Use Helius Enhanced API if available (Avoids RPC 429s)
        if (heliusApiKey) {
            try {
                const fetchWithFallback = async (key) => {
                    const url = `https://api.helius.xyz/v0/addresses/${tokenAddress}/transactions?api-key=${key}&limit=50&type=SWAP`;
                    const res = await fetch(url);
                    if (!res.ok) throw new Error(res.status);
                    return res;
                };
                ...
                let response;
                try {
                    response = await fetchWithFallback(heliusApiKey);
                } catch (err) {
                    const backupKey = process.env.HELIUS_BACKUP_API_KEY;
                    if (backupKey && backupKey !== heliusApiKey) {
                        console.log('Primary Helius key failed (Unique Buyers), using backup...');
                        response = await fetchWithFallback(backupKey);
                    } else {
                        throw err;
                    }
                ...
                console.warn('Helius Enhanced API failed for unique buyers, falling back to signatures:', e);
            }
        }
...
async function getDevFromHelius(tokenAddress, apiKey) {
    try {
        ...
        try {
            ...
            const backupKey = process.env.HELIUS_BACKUP_API_KEY;
            if (backupKey && backupKey !== apiKey) {
                console.log('Primary Helius key failed (Dev Check), using backup...');
                response = await fetchWithFallback(backupKey);
            } else {
                throw e;
            }
