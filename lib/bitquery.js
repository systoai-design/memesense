...
/**
 * Get the very first trades for sniper detection
 * @param {string} tokenAddress 
 */
async function getEarliestTrades(tokenAddress) {
  const client = getBitqueryClient();
  if (!client) return null;

  const query = gql`
    query GetFirstTrades($token: String!) {
      Solana {
        DEXTradeByTokens(
          where: {
            Trade: {
  ...
  try {
    ...
    console.error('Bitquery Snipers error:', error);
    return null;
  }
}
