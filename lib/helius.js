...
/**
 * Smart Fetch Wrapper for Helius
 * Retries EXACTLY ONCE on 429 errors after 1s delay.
...
async function fetchWithSmartRetry(url, options = {}) {
    ...
    // Retry only on 429
    console.warn(`[Helius] 429 Rate Limit. Retrying in 1s...`);
    await sleep(1000);
...
export async function getBatchTokenMetadata(mints) {
    ...
    const chunkPromises = chunks.map(async (chunk) => {
        try {
            const response = await fetchWithSmartRetry(`https://mainnet.helius-rpc.com/?api-key=${apiKey}`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    jsonrpc: '2.0',
