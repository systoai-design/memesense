...
export async function getWalletHistory(walletAddress, maxTxLimit = 1000, sinceTimestamp = null) {
    try {
        ...
        for (const tx of allTxs) {
            ...
            // 1. Calculate Net SOL Change (Equivalent)
            let netSolChange = 0;

            // Check for wSOL Interactions FIRST
            let wsolChange = 0;
            let hasWsolActivity = false;

            incoming.forEach(t => {
                if (t.mint === SOL_MINT) {
                    wsolChange += t.tokenAmount;
                    hasWsolActivity = true;
                }
            });
            outgoing.forEach(t => {
                if (t.mint === SOL_MINT) {
            ...
            // HYBRID LOGIC: 
            // If wSOL is involved, it's likely a Swap where Native Change is just the Wrap/Unwrap.
            // In this case, use wSOL change as the "Real" value, and IGNORE Native Change (to avoid double count).
            // BUT: wSOL change doesn't include Fee. So we must SUBTRACT Fee.

            if (hasWsolActivity) {
                netSolChange += wsolChange;

                // Deduct Fee (User Paid)
                // nativeBalanceChange includes fee, but we are ignoring it.
                // So we manually subtract fee.
                if (tx.feePayer === walletAddress) {
                    netSolChange -= (tx.fee / 1e9);
                }

                // Deduct Jito Tip? (Usually a native transfer to Tip Account)
                // If we ignore Native, we ignore Tip. 
                // We should check "Native Transfers" that are NOT "Wrap" transfers.
                // This is complex. For now, Fee + wSOL is 99% accurate.

            } else {
                // NO wSOL -> Likely Pump.fun or Direct Native Trade.
                // Use Native Balance Change (Includes Fee & Cost).
