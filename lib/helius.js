    ...
    for (let i = 0; i < retries; i++) {
        try {
            ...
            if (res.status === 429) {
                ...
                // Cap the delay at MAX_DELAY (2s) to prevent timeouts
                let delay = Math.min((BASE_DELAY * Math.pow(2, i)) + jitter, MAX_DELAY);

                // Respect Retry-After if present (and reasonable, < 3s)
                ...
                console.warn(`[Helius] 429 Rate Limit. Retrying in ${Math.round(delay)}ms (Attempt ${i + 1}/${retries})...`);
                await sleep(delay);
                continue;
            }
