...
async function fetchWithRetry(url, options = {}, retries = 3, delay = 1000) {
    try {
        ...
        console.warn(`[Helius] Request failed (${err.message}). Retrying in ${delay}ms...`);
 ...
 * @param {number} maxTxLimit - Hard limit for transactions to fetch (Default 100)
 */
export async function getWalletHistory(walletAddress, maxTxLimit = 100) {
    try {
        console.log(`[Helius] Fetching history for ${walletAddress}. Limit: ${maxTxLimit}`);
        ...
        while (page < MAX_PAGES && !reachedLimit) {
            page++;
            // Delay to prevent Rate Limits (200ms)
