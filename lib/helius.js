...
export async function getWalletHistory(walletAddress) {
    try {
        ...
        // Paginated fetch - target 1000 txs or 30 days of history
        const MAX_TXS = 1000;
        const MAX_PAGES = 10; // 10 pages * 100 = 1000 max
        ...
        while (page < MAX_PAGES && !reachedLimit) {
            ...
            const options = {
                limit: 100
                // Removed 'type: SWAP' to catch all trade types
            };
            if (lastSignature) {
            ...
            try {
                ...
                // If 429, maybe wait? But for now just break to return partial data
                break;
            }

            if (!txs || txs.length === 0) break;

            // Check if oldest tx in batch is older than 30 days
            const oldestTx = txs[txs.length - 1];
            if (oldestTx && (oldestTx.timestamp * 1000) < cutoffTime) {
                // Filter only txs within 30 days
                const recentTxs = txs.filter(tx => (tx.timestamp * 1000) >= cutoffTime);
                allTxs = allTxs.concat(recentTxs);
                reachedLimit = true;
                console.log(`[Helius] Reached 30-day cutoff at page ${page}`);
            } else {
            ...
            if (allTxs.length >= MAX_TXS) {
                reachedLimit = true;
                console.log(`[Helius] Reached ${MAX_TXS} tx limit`);
            }
        }
