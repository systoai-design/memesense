...
if (!HELIUS_RPC_URL) {
    console.error('❌ CRITICAL ERROR: No RPC URL configured! Please set HELIUS_RPC_URL in environment variables.');
} else if (HELIUS_RPC_URL.includes('api.mainnet-beta.solana.com')) {
    console.warn('⚠️ WARNING: Using Public RPC (api.mainnet-beta.solana.com). This will likely fail for complex Helius API calls (DAS/Enhanced).');
} else {
    console.log('✅ Helius Lib Initialized with Custom RPC');
}
...
function getApiKey() {
    ...
    if (HELIUS_RPC_URL.includes('api.mainnet-beta.solana.com')) {
        throw new Error('Configuration Error: You are using the Public Solana RPC which does not support Helius API methods. Please add HELIUS_RPC_URL to your Vercel Environment Variables.');
    }
 ...
 */
async function fetchWithBackoff(url, options = {}, retries = 10) {
    const BASE_DELAY = 300;
    const MAX_DELAY = 2000;

    for (let i = 0; i < retries; i++) {
        const controller = new AbortController();
        const timeoutId = setTimeout(() => controller.abort(), 4000); // 4s Nuclear Timeout

        try {
            // Merge options with our abort signal
            // If caller provided a signal, we ideally respect both, but here we prioritize our short fuse.
            // Using logic: if global signal aborts, this aborts. If local timer fires, this aborts.
            ...
            if (res.status === 429) {
                ...
                let delay = Math.min((BASE_DELAY * Math.pow(2, i)) + jitter, MAX_DELAY);

                const retryAfter = res.headers.get('Retry-After');
                if (retryAfter) {
                    const raMs = parseInt(retryAfter) * 1000;
                    if (raMs < 3000) delay = raMs;
                }

                console.warn(`[Helius] 429 Rate Limit. Retrying in ${Math.round(delay)}ms...`);
                await sleep(delay);
                continue;
            }
            ...
            if (err.name === 'AbortError' || err.message.includes('network') || err.message.includes('fetch')) {
                console.warn(`[Helius] Request Failed (${err.name}: ${err.message}). Retrying (Attempt ${i + 1}/${retries})...`);
...
export async function getBatchTokenMetadata(mints) {
    ...
    // Concurrency Limit: 2 parallel requests
    const CONCURRENCY = 2;
    for (let i = 0; i < chunks.length; i += CONCURRENCY) {
        const batch = chunks.slice(i, i + CONCURRENCY);

        // Add delay between batches to respect rate limits
        if (i > 0) await sleep(300); // Increased to 300ms for reliability
...
export async function getWalletHistory(walletAddress, { limit = 100 } = {}) {
    try {
        ...
        while (page < MAX_PAGES && !reachedLimit) {
            page++;
            // Throttle: 300ms to respect Helius Free/Pro tier rate limits
            if (page > 1) await sleep(300);

            // TIME CAP PROTECTION: Stops fetching if we are nearing the 26s limit
            // This ensures we return PARTIAL history rather than crashing with a Timeout
            if (Date.now() - startTime > 18000) { // Stop after 18s (safe buffer)
                console.warn(`[Helius] Time Cap Hit (18s). Returning ${allTxs.length} txs so far.`);
                break;
            }
