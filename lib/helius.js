...
// Retry configuration (optimized for Netlify's 26s limit)
const MAX_RETRIES = 3;  // Max retries (worst case: 1+2+4 = 7s delay)
...
export async function getAsset(assetId) {
    ...
    for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
        try {
            ...
            if (response.status === 429) {
                ...
                console.warn(`[Helius] getAsset Rate limited (429). Retry ${attempt + 1}/${MAX_RETRIES} in ${Math.round(wait)}ms`);
                await sleep(wait);
                continue;
            }
    ...
    // Graceful degradation: return null instead of crashing the request
    console.warn('[Helius] getAsset failed after retries:', lastError?.message);
    return null;
}
...
export async function getEnhancedTransactions(address, options = {}) {
    ...
    for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
        try {
            ...
            if (response.status === 429) {
                const wait = Math.pow(2, attempt) * 1000 + Math.random() * 500;
                console.warn(`[Helius] txHistory Rate limited (429). Retry ${attempt + 1}/${MAX_RETRIES} in ${Math.round(wait)}ms...`);
                await sleep(wait);
                continue;
            }
            if (!response.ok) {
    ...
    // Graceful degradation: return empty array instead of crashing the request
    console.warn('[Helius] getEnhancedTransactions failed after retries:', lastError?.message);
    return [];
}
    ...
    const chunkPromises = chunks.map(async (chunk) => {
        ...
        for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
            try {
                ...
                if (response.status === 429) {
                    const wait = Math.pow(2, attempt) * 1000 + Math.random() * 500;
                    console.warn(`[Helius] Batch metadata rate limited. Retry ${attempt + 1}/${MAX_RETRIES}...`);
                    await sleep(wait);
                    continue;
                }
