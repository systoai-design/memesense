            ...
            if (res.ok) {
                ...
                if (data.pairs) {
                    data.pairs.forEach(pair => {
                        ...
                        if (baseAddress && pair.priceNative) {
                            // 1. If we already found a SOL pair, skip (unless this one is better liquidity?) 
                            // For simplicity, just prioritizing SOL pairs.
                            const current = priceMap[baseAddress];
                            if (current && current.currency === 'SOL') return;

                            let price = 0;
                            let currency = 'SOL';

                            // CASE 1: Quote is SOL -> priceNative is SOL
                            if (quoteAddress === SOL_MINT) {
                                price = parseFloat(pair.priceNative);
                                currency = 'SOL';
                            }
                            // CASE 2: Quote is USDC -> priceNative is USD
                            else if (quoteAddress === USDC_MINT) {
                                price = parseFloat(pair.priceUsd || pair.priceNative);
                                currency = 'USD';
                            }
                            // CASE 3: Unknown Quote (e.g. USDT) -> Fallback to USD
                            else {
                                price = parseFloat(pair.priceUsd);
                                currency = 'USD';
                            }

                            priceMap[baseAddress] = {
                                price,
                                currency,
