...
// Hardcoded premium/admin wallets (bypass database checks)
const PREMIUM_WALLETS = [
  '2unNnTnv5DcmtdQYAJuLzg4azHu67obGL9dX8PYwxUDQ', // Admin
  'FrCFPYpcLpeifKMBqkgq3HMivMDAtb9F2rFFhB5EAsNq'  // New Premium User
];
...
async function canUseAnalysis(userId, depth = 'normal') {
  ...
  // BYPASS FOR HARDCODED PREMIUM WALLETS
  if (user.wallet_address && PREMIUM_WALLETS.includes(user.wallet_address)) {
    return {
      allowed: true,
      remainingToday: 9999,
      isPremium: true
  ...
  // TIER LIMITS CONFIG
  // Updated to align with global TIERS
  if (tierName === 'PREMIUM' || tierName === 'TRIAL' || tierName === 'ADMIN') {
    return {
      allowed: true,
      remainingToday: 9999,
      isPremium: true
  ...
  const TIER_LIMITS = {
    FREE: { normal: 10, deep: 10 }, // Allow 10 scans (Deep or Normal) for Free
  };

  const limits = TIER_LIMITS[tierName] || TIER_LIMITS.FREE;
  const isDeep = depth === 'deep';

  // REMOVED explicit Deep Scan block for Free users. 
  // We now rely on the numeric limit below.
...
/**
 * Initialize User Scans Table (History)
 */
async function initUserScansTable() {
  const db = getClient();
  try {
    await db.execute({
      sql: `CREATE TABLE IF NOT EXISTS user_scans (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
    ...
    await db.execute({
      sql: `CREATE INDEX IF NOT EXISTS idx_user_scans_user ON user_scans(user_id)`
    });
  } catch (e) {
    console.warn('[DB] User scans table init warning:', e.message);
  }
}

/**
 * Log a user scan to history
 */
async function logUserScan(userId, walletAddress, summary = {}) {
  const db = getClient();
  try {
    await initUserScansTable();

    // Get label if exists
    const label = await getWalletLabel(userId, walletAddress).catch(() => null);

    await db.execute({
      sql: `INSERT INTO user_scans (user_id, wallet_address, wallet_label, summary_json) VALUES (?, ?, ?, ?)`,
      args: [userId, walletAddress, label, JSON.stringify(summary)]
    });
    console.log(`[DB] Logged scan for user ${userId} -> ${walletAddress}`);
  } catch (e) {
    console.error('[DB] logUserScan error:', e.message);
  }
}
...
async function getUserScanHistory(userId, limit = 50) {
  ...
  try {
    await initUserScansTable();
    const res = await db.execute({
      sql: `SELECT * FROM user_scans WHERE user_id = ? ORDER BY scanned_at DESC LIMIT ?`,
      args: [userId, limit]
    });
    return res.rows.map(row => ({
      ...row,
    ...
    console.error('[DB] getUserScanHistory error:', e.message);
    return [];
  }
}

/**
 * Clear all stored trades for a wallet (for fresh re-scan)
 */
async function clearWalletTrades(walletAddress) {
  const db = getClient();
  try {
    await initWalletTradesTable();
    await db.execute({
    ...
    console.log(`[DB] Cleared trades for ${walletAddress}`);
    return true;
  } catch (e) {
    console.error('[DB] clearWalletTrades error:', e.message);
    return false;
  }
}
