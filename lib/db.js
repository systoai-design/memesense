...
async function canUseAnalysis(userId) {
  ...
  if (tierName === 'TRIAL' && user.trial_start) {
    ...
    if (diffHours > 72) {
      tierName = 'FREE';
      // Async update to downgrade user permanently so we don't re-check every time
 ...
 */
async function recordScan(userId, tokenData) {
  const db = getClient();
  try {
    await db.execute({
      sql: `INSERT INTO scans (user_id, token_address, name, symbol, image_url) VALUES (?, ?, ?, ?, ?)`,
      args: [
        userId,
        tokenData.address,
        tokenData.name,
...
async function getRecentScans(userId, limit = 10) {
  ...
  try {
    const res = await db.execute({
      sql: `SELECT token_address, name, symbol, image_url, MAX(created_at) as created_at 
            FROM scans 
            WHERE user_id = ?
 ...
 * Upgrade user tier
 */
async function upgradeUser(walletAddress, tier = 'PREMIUM') {
  const db = getClient();
  try {
    const res = await db.execute({
