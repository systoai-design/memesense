...
async function canUseAnalysis(userId) {
  ...
  // Normalize Tier Name (Handle 'Premium' vs 'PREMIUM')
  let rawTier = user.tier || 'FREE';
  let tierName = rawTier.toUpperCase();
  ...
  if (tierName === 'TRIAL' && user.trial_start) {
    ...
    if (diffHours > 72) {
      tierName = 'FREE';
      // SAFEGUARD: Don't auto-write to DB yet, just downgrade status for this session
      // db.execute({ ... }) 
    }
  }

  // CHECK SUBSCRIPTION EXPIRATION (For Monthly)
  // Only check if expiry is a REAL string (not null, not empty, not "null")
  if (tierName === 'PREMIUM' && user.subscription_expiry && user.subscription_expiry !== 'null' && user.subscription_expiry.length > 5) {
    const expiry = new Date(user.subscription_expiry).getTime();
    const now = Date.now();

    if (!isNaN(expiry) && now > expiry) {
      console.log(`User ${userId} subscription expired on ${user.subscription_expiry}. Treating as FREE.`);
      tierName = 'FREE';
      // SAFEGUARD: Disable auto-downgrade for now to prevent bugs
      /* db.execute({
  ...
  // CST Date Logic (America/Chicago)
  // 12:00 AM CST is the reset time. Using 'en-CA' gives YYYY-MM-DD.
  ...
  if (usedToday >= tier.dailyLimit) {
    return {
      allowed: false,
      reason: `Daily limit reached. [DEBUG: Tier=${tierName} (Raw:${rawTier}), Exp=${user.subscription_expiry || 'None'}, Used=${usedToday}/${tier.dailyLimit}]`,
      remainingToday: 0,
      usedToday,
...
async function recordScan(userId, tokenData, type = 'token') {
  const db = getClient();
  try {
    // Attempt to update timestamp if already exists
    // We wrap this in a try-catch to handle potential missing 'type' column
    let updateRes;
    try {
      updateRes = await db.execute({
        sql: 'UPDATE scans SET created_at = CURRENT_TIMESTAMP WHERE user_id = ? AND token_address = ? AND type = ?',
        args: [userId, tokenData.address, type]
      });
    } catch (sqlErr) {
      if (sqlErr.message.includes('no such column') || sqlErr.message.includes('type')) {
        console.log("Adding missing 'type' column to scans table...");
        await db.execute({ sql: "ALTER TABLE scans ADD COLUMN type TEXT DEFAULT 'token'" });
        // Retry update
        updateRes = await db.execute({
          sql: 'UPDATE scans SET created_at = CURRENT_TIMESTAMP WHERE user_id = ? AND token_address = ? AND type = ?',
          args: [userId, tokenData.address, type]
...
async function getRecentScans(userId, limit = 10, type = 'token') {
  ...
  try {
    ...
    try {
      ...
      if (sqlErr.message.includes('no such column') || sqlErr.message.includes('type')) {
        console.log("Adding missing 'type' column during fetch...");
        await db.execute({ sql: "ALTER TABLE scans ADD COLUMN type TEXT DEFAULT 'token'" });
        // Retry fetch
        res = await db.execute({
          sql: `SELECT s.token_address, s.name, s.symbol, s.image_url, MAX(s.created_at) as created_at,
